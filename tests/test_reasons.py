...
def test_date_pair_replacement_is_proved_by_returning_the_busy_competitor_alone(
    ...
    for card, reason in replacements:
        ...
        assert reason.evidence["scarcity"].startswith("в эту дату свободны ")
...
def test_unique_closest_description_without_tags_is_support_only(
    ...
    result, scored, scorer = reason_input(catalogue, match_request)
    assigned = assign(result, scored, catalogue, scorer)
    # With a tag the description family may surface as DESCRIPTION_ASPECT (the closest-in-shown
    # variant is then redundant); without tags only the support-only closest reason exists.
    codes = {"DESCRIPTION_CLOSEST_IN_SHOWN", "DESCRIPTION_ASPECT"} if tagged else {"DESCRIPTION_CLOSEST_IN_SHOWN"}
    closest = next(r for c in assigned.cards for r in c.reasons if r.code in codes)
